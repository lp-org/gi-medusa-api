...
class SESService extends NotificationService {
  ...
  async sendNotification(event, eventData, attachmentGenerator) {
    ...
    if (!templateId) {
      throw new MedusaError(
        ...
        `SES service: No template was set for event: ${event}`
      );
    }
    ...
    if (!data) {
      throw new MedusaError(
        MedusaError.Types.INVALID_DATA,
        "SES service: Invalid event data was received"
      );
    }
  ...
  async compileTemplate(templateId, data) {
    const storeContent = await this.storeContentService_.get();
                    ...
                    <table width="100%" align="center" role="presentation" cellSpacing="0" cellPadding="0" border="0">
                      <tbody style="width:100%">
                        <tr style="width:100%">
                          ...
                          <td>
                            <p style="font-size:13.5px;line-height:24px;margin:16px 0;margin-top:0;font-weight:500;color:#000;margin-bottom:0">${
    ...
    <hr style="width:100%;border:none;border-top:1px solid #eaeaea;border-color:#E5E5E5;margin:0" />
      ...
      <tbody>
        <tr>
          <td>
            <p style="font-size:32px;line-height:1.3;margin:16px 0;font-weight:700;text-align:center;letter-spacing:-1px">${
              storeContent.store.name
            }</p>
      ...
      <tbody>
        <tr>
          <td>
            ...
            <p style="font-size:13px;line-height:24px;margin:0;color:#AFAFAF;text-align:center">© 2023 ${
              storeContent.store.name
